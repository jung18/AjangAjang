  ...
  useEffect(() => {
    if (rooms.length > 0) {
      ...
      const socket = new SockJS(
        `https://i11b210.p.ssafy.io:4443/ws-stomp?token=${accessToken}`
      );
          ...
          rooms.forEach((room) => {
            client.subscribe(`/sub/chat/${room.id}`, (msg) => {
              ...
              const lastReadTime = new Date(
                newMessages[room.id]?.lastReadTime || room.lastReadTime
              );

              setNewMessages((prev) => {
                const newUnreadCount =
                  !isCurrentRoom && new Date(message.time) > lastReadTime
                ...
                <div className={styles["room-header"]}>
                  ...
                  {newMessages[room.id]?.unreadCount > 0 && (
                    <div className={styles["unread-indicator"]}>
                      {newMessages[room.id].unreadCount}
                    </div>
                  )}
                </div>
                <div className={styles["room-last-message"]}>
                  <span>
                    {newMessages[room.id]?.lastMessage || room.lastMessage}
                  </span>
                </div>
              </div>
            </li>
